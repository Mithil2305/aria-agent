...
export default function ActionableForecast({ token, analysisReady }) {
  ...
  const fetchForecasts = async () => {
    ...
    try {
      ...
      setForecasts(data.forecast_actions || []);
    } catch (err) {
  ...
  if (loading) {
    return (
      <div className="card p-5 flex items-center gap-3">
        ...
        <span className="text-sm text-surface-500">Analysing forecast trends…</span>
      </div>
    );
  }
    ...
    <div className="card p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold text-surface-900">What to Expect Next</h3>
        </div>
        <button
      ...
      <p className="text-xs text-surface-500 mb-4">
        Based on your data trends — here's what's coming and what to do about it.
      </p>
            ...
            >
              <div className="flex items-start gap-3">
                <div className="flex items-center gap-1.5 shrink-0 mt-0.5">
                  <span className={`w-2 h-2 rounded-full ${URGENCY_DOTS[fc.urgency] || "bg-surface-400"}`} />
                  <span className="text-lg leading-none">{fc.icon}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-1.5 flex-wrap">
                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${style.badge}`}>
                      {style.badgeText}
                    </span>
                  </div>
