...
function QuickStatsBar({ avgRevenue, avgCustomers, avgOrders, dataPoints }) {
	const stats = [
		...
		},
		{
			...
			value:
				avgCustomers != null ? String(Math.round(Number(avgCustomers))) : "0",
			icon: Users,
		},
		{
			label: "Avg Orders/Day",
			value: avgOrders != null ? String(Math.round(Number(avgOrders))) : "0",
			icon: ShoppingCart,
		},
		{
			label: "Data Points",
			value: dataPoints != null ? String(dataPoints) : "0",
			icon: Database,
		},
	];
			...
			{stats.map((s, i) => {
				...
				return (
					<div key={i} className="card p-4">
						<p className="text-[10px] text-surface-400 font-medium uppercase tracking-wider mb-1.5">
							{s.label}
						</p>
...
function HealthBadge({ score }) {
	...
	return (
		<div
			className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border ${bg}`}
		>
...
function TrendBadge({ trendLock }) {
	...
	return (
		<div
			className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg border ${isUp ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
		>
			{isUp ? (
			...
			>
				{trendLock.metric} {isUp ? "up" : "down"}{" "}
				{Math.abs(trendLock.change || 0)}%
			</span>
		</div>
	);
}
...
function SectionHeader({ icon: Icon, title, subtitle }) {
	return (
		<div className="mb-5">
			...
			{subtitle && (
				<p className="text-xs text-surface-500 mt-0.5 ml-7">{subtitle}</p>
			)}
		</div>
	);
}
...
function MetricBox({
	...
	return (
		<div
			className={`rounded-lg p-4 ${highlight ? "bg-green-50 border border-green-200" : "bg-surface-100"}`}
		>
			...
			{change != null && !highlight && (
				<div
					className={`flex items-center gap-1 text-xs font-semibold mt-1 ${isNeutral ? "text-surface-400" : isUp ? "text-green-600" : "text-red-500"}`}
				>
					{isNeutral ? (
						<Minus size={11} />
					) : isUp ? (
	...
	return (
		<div className="space-y-6">
			<SectionHeader
				icon={Wallet}
				title="Daily Khata"
				subtitle="Your financial health at a glance. How to earn more profit?"
			/>
			{revenueKpi && (
				<div className="card-elevated p-5">
					<div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
						...
						{expenseKpi && (
							<MetricBox
								label="Expenses"
								value={expenseKpi.current}
								change={expenseKpi.change}
								isRupee
								invertColor
							/>
						)}
						{expenseKpi && (
							<MetricBox
								...
								value={revenueKpi.current - expenseKpi.current}
								isRupee
								highlight
							/>
						)}
...
function FootfallSection({ kpis, insights }) {
	...
	return (
		<div className="space-y-6">
			<SectionHeader
				icon={Footprints}
				title="Footfall & Orders"
				subtitle="Customer traffic patterns and order trends"
			/>
				...
				<div className="card-elevated p-5">
					<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
						{customerKpi && (
							<MetricBox
								label="Customers"
								value={customerKpi.current}
								change={customerKpi.change}
							/>
						)}
						{orderKpi && (
							<MetricBox
								label="Orders"
								value={orderKpi.current}
								change={orderKpi.change}
							/>
						)}
					</div>
				</div>
			...
			<div className="space-y-3">
				...
				{footfallInsights.length === 0 && (
					<EmptyState text="Log customer and order data to see footfall insights." />
				)}
			</div>
		</div>
	);
	...
	return (
		<div className="space-y-6">
			<SectionHeader
				icon={Warehouse}
				title="Smart Godown"
				subtitle="Inventory efficiency. What's overstocked or running low?"
			/>
			{invKpi && (
				<div className="card-elevated p-5">
					<MetricBox
						label="Current Inventory"
						value={invKpi.current}
						change={invKpi.change}
					/>
				</div>
			)}
			<div className="space-y-3">
				...
				{godownInsights.length === 0 && (
					<EmptyState text="Log your inventory and stock data to see godown insights." />
				)}
			</div>
		</div>
	);
	...
	return (
		<div className="space-y-6">
			<SectionHeader
				...
				subtitle="Who are your customers? How to keep them coming back?"
			/>
			{basketKpi && (
				<div className="card-elevated p-5">
					<MetricBox
						label="Average Bill Value"
						value={basketKpi.current}
						change={basketKpi.change}
						isRupee
					/>
				</div>
			)}
			<div className="space-y-3">
				...
				{customerInsights.length === 0 && (
					<EmptyState text="Log customer data and basket size to see customer insights." />
				)}
			</div>
			{correlations.length > 0 && (
				<div className="card p-5">
					<h3 className="text-sm font-medium text-surface-900 mb-3 flex items-center gap-2">
						<Link2 size={14} className="text-indigo-500" /> How Your Numbers
						Connect
					</h3>
					<div className="space-y-2">
						{correlations.slice(0, 5).map((c, i) => (
							<div
								key={i}
								className="flex items-center gap-3 px-3 py-2 rounded-lg bg-surface-100 text-xs"
							>
								<span className="font-medium text-surface-700">{c.label1}</span>
								<span
									className={`text-xs ${c.direction === "positive" ? "text-green-600" : "text-red-500"}`}
								>
									{c.direction === "positive" ? (
	...
	return (
		<div className="space-y-6">
			<SectionHeader
				icon={BrainCircuit}
				title="Yukti Advisor"
				subtitle="AI-powered action plan for your business this week"
			/>
			{trendLock?.direction && (
				<div
					className={`card-elevated p-5 border-l-4 ${trendLock.direction === "up" ? "border-l-green-500" : trendLock.direction === "down" ? "border-l-red-500" : "border-l-surface-400"}`}
				>
			...
			<div className="space-y-3">
				...
				{advisorInsights.length === 0 && (
					<EmptyState text="Run an analysis with more data to get personalized advisor insights." />
				)}
			</div>
	...
	return (
		<div className="space-y-6">
			<SectionHeader
				icon={LineChart}
				title="Predictions"
				subtitle="Where your business is heading based on your data"
			/>
			{dataSufficiency !== "full" && (
					...
					<div className="flex items-start gap-3">
						...
						<div>
							...
							<p className="text-xs text-amber-600 mt-1">
								Log 14+ days of data to unlock powerful predictions. The more
								data you provide, the smarter Yukti gets.
							</p>
						</div>
					</div>
				</div>
					...
					{forecasts.slice(0, 6).map((f, i) => {
						...
						return (
							...
							>
								...
								<span
									className={`px-2 py-0.5 rounded-full text-[10px] font-medium ${confidenceColor}`}
								>
	...
	return (
		<div className="space-y-6">
			<SectionHeader
				...
				subtitle="Things that look different from your normal pattern"
			/>
			{anomalies.length === 0 ? (
				<div className="card-elevated p-8 text-center">
					<CheckCircle2 size={32} className="text-green-500 mx-auto mb-3" />
					<h3 className="text-sm font-medium text-surface-900 mb-1">
						All Clear
					</h3>
					<p className="text-xs text-surface-500">
						All your numbers are within normal range. Nothing unusual today.
					</p>
				</div>
			) : (
				<div className="space-y-3">
					{anomalies.map((a, i) => {
						...
						return (
							<div
								key={i}
								className={`card p-4 border-l-4 ${isCritical ? "border-l-red-500 bg-red-50" : isHigh ? "border-l-amber-500 bg-amber-50" : "border-l-surface-400"}`}
							>
								<div className="flex items-start gap-3">
									<AlertTriangle
										size={16}
									...
									<div>
										<p className="text-sm font-medium text-surface-800">
											{a.label || a.column}:{" "}
											{a.type === "spike" ? "Unusually high" : "Unusually low"}
										</p>
										<p className="text-xs text-surface-600 mt-0.5">
											{Math.abs(a.deviation)}% different from your normal
											pattern.
											{isCritical
	...
	return (
		<div className="space-y-6">
			<SectionHeader
				...
				subtitle={`AI-powered tips tailored for ${categoryLabel || "your business"}`}
			/>
						...
						<div className="space-y-3 flex-1">
							<div>
								...
								<p className="text-xs text-surface-500 mt-1 leading-relaxed">
									Yukti will analyse your daily logs, revenue trends, and stock
									data to generate personalised strategies including sales tips,
									customer acquisition ideas, stock optimisation, and a monthly
									roadmap.
								</p>
							</div>
			...
			{strategy && (
				<>
					...
					{stratTab === "sales" && (
						<div className="space-y-3">
							<h3 className="text-sm font-medium text-surface-700 flex items-center gap-2">
								<TrendingUp size={15} className="text-indigo-500" /> Sales
								Improvement Tips
							</h3>
							...
							{(!strategy.salesTips || strategy.salesTips.length === 0) && (
								<EmptyState text="No sales tips available yet." />
							)}
						</div>
					)}
					...
					{stratTab === "customers" && (
						<div className="space-y-3">
							<h3 className="text-sm font-medium text-surface-700 flex items-center gap-2">
								<Users size={15} className="text-indigo-500" /> Customer
							...
							{(!strategy.customerStrategies ||
								strategy.customerStrategies.length === 0) && (
								<EmptyState text="No customer strategies available yet." />
							)}
						</div>
					)}
						...
						<div className="space-y-6">
							<div>
								<h3 className="text-sm font-medium text-surface-700 flex items-center gap-2 mb-3">
									<AlertCircle size={15} className="text-amber-500" /> Why Stock
									is Pending
								</h3>
								<div className="card-elevated p-5 space-y-3">
									...
									{(!strategy.stockAnalysis?.pendingReasons ||
										strategy.stockAnalysis.pendingReasons.length === 0) && (
										<p className="text-xs text-surface-400">
											No pending stock issues found.
										</p>
									)}
								</div>
							</div>
							<div>
								<h3 className="text-sm font-medium text-surface-700 flex items-center gap-2 mb-3">
									<CheckCircle2 size={15} className="text-green-500" /> Stock
								...
								<div className="card-elevated p-5 space-y-3">
									...
									{(!strategy.stockAnalysis?.recommendations ||
										strategy.stockAnalysis.recommendations.length === 0) && (
										<p className="text-xs text-surface-400">
					...
					{stratTab === "purchase" && (
						<div className="space-y-3">
							...
							{(!strategy.purchaseSuggestions ||
								strategy.purchaseSuggestions.length === 0) && (
								<EmptyState text="No purchase suggestions available yet." />
							)}
						</div>
					)}
					...
					{stratTab === "roadmap" && (
						<div className="space-y-4">
							<h3 className="text-sm font-medium text-surface-700 flex items-center gap-2">
								<Calendar size={15} className="text-indigo-500" /> 4-Week Action
								Roadmap
							</h3>
							...
							{(!strategy.roadmap || strategy.roadmap.length === 0) && (
								<EmptyState text="No roadmap available yet." />
							)}
						</div>
					)}
				</>
					...
					return (
						...
						>
							...
							<div className="text-right shrink-0 ml-3">
								...
								<span
									className={`text-[10px] font-semibold ${isNeutral ? "text-surface-400" : isUp ? "text-green-600" : "text-red-500"}`}
								>
									{isUp ? "+" : ""}
									{kpi.change}%
								</span>
