...
function BenchmarkRow({ label, yourValue, benchmarkValue, isRupee, higherIsBetter = true, icon: Icon }) {
  const hasData = yourValue != null && yourValue > 0;
  const ratio = hasData && benchmarkValue > 0 ? yourValue / benchmarkValue : null;
  const isGood = ratio != null ? (higherIsBetter ? ratio >= 0.9 : ratio <= 1.1) : null;
  const isWarn = ratio != null ? (higherIsBetter ? ratio < 0.7 : ratio > 1.3) : null;

  const statusColor = isGood ? "text-green-600" : isWarn ? "text-red-500" : "text-amber-500";
  const barWidth = ratio != null ? Math.min(Math.round(ratio * 100), 140) : 0;
  const barColor = isGood ? "bg-green-500" : isWarn ? "bg-red-400" : "bg-amber-400";

  const format = (v) => (isRupee ? formatINR(v) : v?.toLocaleString("en-IN") || "—");
    ...
    <div className="py-3 border-b border-surface-100 last:border-0">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex items-center gap-3 text-xs">
          <span className={`font-bold ${hasData ? statusColor : "text-surface-400"}`}>
  ...
  if (loading) {
    return (
      <div className="card p-5 flex items-center gap-3">
        ...
        <span className="text-sm text-surface-500">Loading market benchmarks…</span>
      </div>
    );
  }
  ...
  return (
    <div className="card p-5">
      ...
      <p className="text-xs text-surface-500 mb-4">
        Market averages for {categoryName.toLowerCase()} in your region. The bar shows your performance vs benchmark.
      </p>
      ...
      <div>
        <BenchmarkRow
          ...
          benchmarkValue={benchmarks.avg_daily_revenue}
          isRupee
          icon={BarChart3}
        />
        <BenchmarkRow
          ...
          benchmarkValue={benchmarks.avg_customers_per_day}
          icon={Users}
        />
        <BenchmarkRow
          label="Avg Basket Size"
          yourValue={basketKpi?.current || (revenueKpi?.current && customerKpi?.current ? revenueKpi.current / customerKpi.current : null)}
          benchmarkValue={benchmarks.avg_basket_size}
          isRupee
          icon={ShoppingCart}
        />
        <BenchmarkRow
          ...
          benchmarkValue={benchmarks.avg_margin_pct}
          icon={TrendingUp}
        />
      </div>
      ...
      <div className="mt-4 px-3 py-2.5 rounded-lg bg-indigo-50 border border-indigo-200">
        ...
        <p className="text-[11px] text-indigo-600 mt-0.5">
          Peak hours: <strong>{benchmarks.peak_hours}</strong> · Best days: <strong>{benchmarks.peak_days}</strong>
        </p>
      </div>
    </div>
  );
